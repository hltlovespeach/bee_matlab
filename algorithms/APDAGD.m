%%
% SUMMARY: Function for the Adaptive Primal-Dual Accelerated Gradient 
% Descent(AGD) algorithm.

% NOTE: Modified so uses both round_transpoly and round_matrix

% REFERENCE: https://github.com/chervud/AGD-vs-Sinkhorn

function [iter,time,otvals]=APDAGD(Ai,r,c,C,epsilon,compute_otvals,max_iter)
...
X =Ai;
combinedSums = [sum(X, 2); sum(X, 1)'];


if compute_otvals
    % initialize OT
    otvals = zeros(max_iter+1,1);
    % size(C) 784*784
    otvals(1) = frobinnerproduct(round_transpoly(X,r,c'),C);
end
    ...
    for k = 1 : 1 : (maxIter - 1)
        ...
        while flag > 0
            ...
            a_t = (1  + sqrt(1 + 4 * L_t * A(k, 1)) )/ 2 / L_t ; 
            % trial for calculate a_k as solution of quadratic equation explicitly
            ...
            l = psi_x_t + mtimes(grad_psi_x_t',y_t - x_t) + L_t / 2 * norm(y_t - x_t,2)^2;
            %calculate r.h.s. of the stopping criterion in the inner cycle
        ...
        otvals(k+1) = frobinnerproduct(X_hat,C);
    ...
    str = ['average time per iteration ',num2str(toc/k),', time ',num2str(toc,3),', iterations ',num2str(k)];
    disp(str); %print current iteration number
    disp(num2str(error_constr)) %print current error
